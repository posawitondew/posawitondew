...
// ambil data (query) dari tabel alternatif_saw
$alternatif = query("SELECT * FROM alternatif");

//tombol ditekan
if( isset($_POST["cari"]) ) {
	$alternatif = cari($_POST["keyword"]);
}
?>
...
<body>
    ...
    <div class="container-fluid page-header mb-5 position-relative overlay-bottom">
        <div class="d-flex flex-column align-items-center justify-content-center pt-0 pt-lg-5" style="min-height: 600px">
            <h1 class="display-4 mb-3 mt-0 mt-lg-5 text-white text-uppercase">Alternatif</h1>
            <div class="d-inline-flex mb-lg-5">
                <p class="m-0 text-white"><a class="text-white" href="">SPK Pupuk</a></p>
                <p class="m-0 text-white px-2">/</p>
                <p class="m-0 text-white">Alternatif</p>
            </div>
        </div>
    </div>
    <!-- Page Header End -->
	 ...
	 <!-- Service Start -->
	<h2> Daftar Nama Alternatif </h2>
	<a href="tambahdata_alternatif.php"> Tambah Data</a>
	<br>
	<br>
	...
	<center><table border="1" cellpadding="10" cellspacing="0">
		...
		<?php $no = 1; ?>
		<?php foreach ($alternatif as $alt) :$no++; ?>
		<tr>
			<td> <?= $no++ ?></td>
			<td> <?=$alt["alternatif"]; ?> </td>
			<td> <?=$alt["keterangan"]; ?> </td>
			 <td><a href="editalternatif.php?id=<?= $alt ["id"]; ?> ">Edit | <a href="hapus_alternatif.php?id=<?= $alt ["id"]; ?> " onclick= "return confirm ('Anda yakin ingin mengapus data ini?')" >Hapus</a></td>
		</tr>
		<?php $no++; ?>
		<?php endforeach ; ?>
	</table></center>
